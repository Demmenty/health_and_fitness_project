...
class FatsecretCacheManager():
    ...
    def __init__(self):
        self.food_info_cache = str(CURRENT_DIR) + "/cache/food_info_cache.pickle"
        self.daily_total_cache = str(CURRENT_DIR) + "/cache/daily_total_cache.pickle"
        self.monthly_total_cache = str(CURRENT_DIR) + "/cache/monthly_total_cache.pickle"
    ...
    def _caches_exist(self) -> bool:
        ...
        if (Path.exists(CURRENT_DIR / 'cache' / 'food_info_cache.pickle') and
            Path.exists(CURRENT_DIR / 'cache' / 'daily_total_cache.pickle') and 
            Path.exists(CURRENT_DIR / 'cache' / 'monthly_total_cache.pickle')):
    ...
    def save_foodmetric(self, prods_without_info) -> None:
        ...
        for i in range(count_of_prods):
            ...
            if type(cache[food_id]['servings']['serving']) is dict:
                cache[food_id]['servings']['serving']["metric_serving_amount"] = metric_serving_amount
                cache[food_id]['servings']['serving']["metric_serving_unit"] = metric_serving_unit
            else:
    ...
    def save_daily_total(self, user, entry_date: datetime, daily_total:dict) -> None:
    ...
    def get_monthly_total(self, user, entry_month: datetime) -> Union[dict, None]:
        """добыча из кеша суммарного количества и калорий съеденной еды
    ...
    def save_monthly_total(self, user, entry_month: datetime, monthly_total:dict) -> None:
        """сохранение в кеше суммарного количества и калорий
        съеденной еды по названиям за один день для ускорения создания топов
        записываются даты старше 2 дней и единожды
        """
