...
def link_fatsecret(request):
    ...
    if oauth_verifier is None:
        auth_url = fatsecret.session.get_authorize_url(callback_url)
        fatsecret.save_session()
        return redirect(auth_url)

    # Step 2: Authenticate the user with FatSecret and save the tokens (session must be the same!)
    try:
        fatsecret.load_session(request.user.id)
        tokens = fatsecret.session.authenticate(oauth_verifier)
        fatsecret.save_tokens(client_tokens=tokens, client=request.user)
