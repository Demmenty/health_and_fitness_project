...
class Command(SuperUserCommand):
    help = "Used to create an expert."

    def handle(self, *args, **options):
        username = options[self.UserModel.USERNAME_FIELD]
        database = options["database"]
        user_data = {"is_expert": True}
        ...
        try:
            if options["interactive"]:
                ...
                if PASSWORD_FIELD in user_data and "EXPERT_PASSWORD" in os.environ:
                    user_data[PASSWORD_FIELD] = os.environ["EXPERT_PASSWORD"]
                # Use username from environment variable, if not provided in
                # options.
                if username is None:
                    username = os.environ.get(
                        "EXPERT_" + self.UserModel.USERNAME_FIELD.upper()
                    )
                if username is None:
                    raise CommandError(
                ...
                user_data[self.UserModel.USERNAME_FIELD] = username
                for field_name in self.UserModel.REQUIRED_FIELDS:
                    env_var = "EXPERT_" + field_name.upper()
            ...
            if options["verbosity"] >= 1:
                self.stdout.write("Expert created successfully.")
            ...
            self.stdout.write(
                "Expert creation skipped due to not running in a TTY. "
                "You can run `manage.py createexpert` in your project "
